...
**Answer**
the amount field is <dbl> and the name field is <chr>.
```{r}
...
**Answer**:
Subway has  franchises 275, when the name is NA there are 192959.
```{r}
...
**Answer**:
it is still the same amount of 135073, i can conclude that running the two different codes provides the same outcome so there are multiple ways to get the correct answer kind of like math. 
```{r}
ppp_maryland_loans %>%
...
**Answer**
There are no loans for october 2020.
```{r}
ppp_maryland_loans %>%
...
**Answer**
the largest loan is for $10000000, and the smallest is $6. The largest loan is suspicious because that is a huge chunk of money but what is more suspicious is the loan for 6, that doesnt make sense to get a loan for such little money.
```{r}
...
**Answer**
Yes they align, the initial approval amount matches the amount they recieved.
...
**Answer**
The amount column is a character and not a number, the column headers are inconsistantly styled, and there is a fully duplicated row.
```{r}
arnold_md_loans
```
...
Im not sure exactly but i think it means there was a probem summarising the columns of total amounts.
```{r}
# cleaning function
...
**Answer**
There are 2 duplicate rows for Stuart Jones LLC.
```{r}
cleaned_arnold_md_loans %>%
  get_dupes()
```
