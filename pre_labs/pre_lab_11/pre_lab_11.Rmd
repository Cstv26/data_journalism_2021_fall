...
**Answer**
A creenshot of different text files.
```{r, echo=FALSE}
...
**Answer**
33 columns, object id, id , name, address, city, state, zip, zip4, telephone, type, status, population, county, countyfips, country, latitude, longitude, naics_code, naics_desc, source, sourcedate, val_method, val_date, website, state_id,alt_name, st_fips, owner, ttl_staff, beds, trauma, helipad, geometry. 
```{r}
...
**Answer**
All info about the MD hospitals in the data set.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The amount of each type of hospital in md in the data.
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Showed the data of general acute care hospitals in md.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Glimpses the dataframe of the counties.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Filtered so the new data frame only had the md counties.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The scatterplot of the hospitals are now on the map.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The color of the dot is shown by the number of beds the hospital. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Shows me the md hospital with general acite care data.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It shows the number of beds in each county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Shows the population of each county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Data that is arranged by beds by county largest to smallest.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Shows the first 10 data pieces in the set.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Joined it with the bed data, now shows the first 10 witht hte most beds per 100k.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Replaced Queen Anne's NA with 0.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Showed the number of beds per 100k by mutating, and then took out Queen Annes.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Showed the md counties by color by amount of beds per 100k.
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Changed the color and removed the amount of 0 from the key.
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
Made the range more specific on the key by what was in the data, and showed gray counties with no beds.
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
