...
```{r}
knitr::include_graphics(rep("images/stevens-lender.png"))
```
...
The code made a data frame od MD counties and how many loans there where in each.
```{r}
...
**Answer** Describe what the code below is doing and the output.
The count of loans is now greater than 6, removing 8 outliers.
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
Both data frams are together now.
```{r}
...
It zoomed into just Maryland where I can see different colored counties.
...
**Answer** Describe the changes from the previous map
Now when you hover over a county, it tells you the name and count.
...
```{r}
knitr::include_graphics(rep("images/pppmap.png"))
```