...
**Answer**
Error: Can't join on `x$zip` x `y$zip` because of incompatible types. ℹ `x$zip` is of type <character>>. ℹ `y$zip` is of type <double>>. Im guessing this means they are different types of data so they cant be joined. Its trying to match a number to a word.
```{r}
...
**Answer**
Rural average is $132475.66, and the median is $27713.5. The average urban amount is $92845.15, and the median amount is $20800.
```{r}
...
**Answer**
The rural average amount is $81815.26 and the median amount is $24675. The average urban amount is $100197.98, and the median amount is $25225.
```{r}
