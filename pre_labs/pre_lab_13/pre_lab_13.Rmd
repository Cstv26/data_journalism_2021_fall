...
**Answer**
It shows 51 rows (states) and whether they voted for trump or clinton and their ppp loan percentage.
```{r}
...
**Answer**
A positive correlation to states voting for trump and requesting PPP loans.
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
Now the y is ppp amount per employee.
```{r}
...
**Answer**
3 rows and 2 colums showing the data on trump voters to ppp loan amounts.
```{r}
reveal_data %>%
...
**Answer**
It shows the p value, t score, and df of the data, as well as the confidence interval and correlation.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
The same data types as before but with ppp amount per employee.
```{r}
...
**Answer**
Shows the mean ppp amount per employee and the mean businesses receiving ppps per candidate.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
A two sample t-test to calculate the mean ppp amount per employee per candidate. 
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
A two sample t-test to calculate the mean amount of bussinesses receiving ppps per candidate. 
```{r}
