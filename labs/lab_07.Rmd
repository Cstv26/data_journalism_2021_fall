...
A1.
89 individual cases were filed by the U.S. Department of Justice for accusations of fraud related to the PPP or EIDL loan program.
```{r}
...
A2.
There were 28 individuals judicial districts that filed fraud cases regaurding the PPP and EIDL loan programs.
```{r}
url<-"https://www.justice.gov/criminal-fraud/cares-act-fraud?"
...
A4.
Smith is a South Florida Rapper who was accused of using fraudulent loan money to buy a Ferrari. He got almost $500,000 for his company using fraudulent paper work. I would have assigned someone to do this after the complaints have gone through to make sure we could get as much accurate information as possible. 
...
A5.
29.79% of all judicial districts filled cases against people.
```{r}
...
A6.
The reason that these judicial districts filed cases could be becuase they had higher populations leading to more chances of fraud. The issue here would be just population distribution.
...
url<-"https://smussenden.github.io/coursefiles/ppp_scraping_example/states/alabama.html"
...
for(row_number in 1:nrow(state_urls)){
  ...
  indvidual_url <- each_row_df$url 
  
  state_table <- indvidual_url %>%    
    read_html() %>%
    html_table() 
