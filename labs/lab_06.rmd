...
**A1** 
The Suburban Washington region had the highest percentage of 62.84% and Montgomery County had 36.79%.
```{r}
# Write the code to produce your answer here
international_migration <-international_migration %>%
mutate(percent_of_total=Total/198996) %>%
  group_by(county)%>%
...
**A2** The data we produced was a percent of total for each region and county in Maryland. Suburban Washingotn had the highest percentage of international migration which makes sense since it is the nation's capital with an extremely diverse populaion. 
...
**A3** Cecil had a percent change of -127%.

```{r}
# Write the code to produce your answer here
international_migration <-international_migration %>%
mutate(percent_change=(July_2018-July_2017)/July_2017)
```

**Q4** What's your best guess as to why these changes occurred, and in those places in particular?
**A4** The changes in laws on immigration probably influenced this change, since it was during Donald Trump's Presidency. 
## Back to Tabula
...
**A5** Baltimore City had a net lose of -62834.
...
**A6** 12 Jurisdictions and 2 reigions had a net positive migration for July 2017, July 2018 and July 2019.

```{r}
# Write the code to produce your answer here
domestic_migration %>%
  filter(July_2017>0 & July_2018>0 & July_2016>0)
```

**Q7** How would you describe this data? Is there a county or region that stands out, and why?
**A7** This data is kind of all over the place with negative and positive  values everywhere. Caroline county stands out to me in the data because its total migration is in the negatives, while the last 3 months are in the net positive. Also Charles, Worcester, and Howard counties remained positive this entire time.
