...
library(tidyverse)
library(tidycensus)
wv_state <- read_rds("data/lab_09.rds")
```
...
```
**A1.** 
April 28th 2020 saw the most loans.
...
sf1 <- load_variables(2019, "acs5", cache = TRUE)
View(sf1)
...
State<-New_ACS %>%
  left_join(state_new, by=c("state"))%>%
...
State %>%
  ggplot()+
  geom_bar((aes(x=reorder(state,total_ppp_loans), weight=change)),fill="blue")+
  coord_flip()
```
**A2.** 
