...
```{r}
maryland_ppp_naics_codes <- maryland_ppp %>% left_join(naics_codes, by="naics_code") %>% mutate(zip5 = str_sub(zip, start=1L, end=5L))
```
...
```{r}
maryland_ppp_with_naics_and_zcta<- maryland_ppp_naics_codes%>% left_join(maryland_zcta,by=c("zip5"="ZCTA5N"))
```
...
**A2** Hotels, Electrical contractors, Advertising agencies, Full-service restaurants, and Elementary and secondary schools.
...
**A3** 20850 has the largest sum of loans with a count of 3186 loans.
```{r}
maryland_ppp_with_naics_and_zcta %>%
  filter(NHA > 20) %>%
   group_by(zip5)%>%
...
**A5** 
The reason that only 240 applicants were asians could be either not many asians needed to apply, or maybe their race affected thier approval. 
...
**A6**  3 zip codes, 	20701, 21031, 21240. The reason this might be hapening is because the people who identify as asian might not live in heavily asian populated areas.
```{r}
maryland_ppp_with_naics_and_zcta %>%
  filter(race == "Asian") %>%
  group_by(zip5) %>%
