...
wv_pop <- read.csv("data/wv_population_county.csv")
zip_25401 <- read.csv("data/zip_25401_loan_sample.csv")
...
**A1.**
126 E Burke Way had the most loans of 6.The businesses are DREW HOLDINGS LLC, BRIX27 LLC, ABRAHAM ASHTON, HUB CO-OP LLC, and RONIN PROPERTIES LLC. 
```{r}
new_zip_25401 <- read.csv("data/zip_25401_loan_sample-csv.csv")
...
**A2.**
It is odd that all of these businesses at the same address needed loans, but when looking up the actual address itself I found that this is a community workspace, so multiple bussiness are at that address, so I do not think it is suspicious. 
...
**A3.**
Martinsburg had the highest number of loans at 1166 loans. Full Service restaurants had the most loans of 50.
```{r}
  Berkeley_loans <- lab_05 %>%
  filter(project_county_name == "BERKELEY") 
 write_csv(Berkeley_loans,"data/Berkeley_loans.csv")
 
 clean_berk <- read.csv("data/cleaned_Berkeley_loans-csv.csv")
 ...
 clean_burke_naics <- clean_berk %>% left_join(naics_codes, by=c("naics_code"))
...
**A4.** 
Full Service restuarants are at the top of the list because:
1. No one was going out to eat because of quarantine
~or~
2.Doordash and Ubereats were taking over during that time so they had to get money to somehow pay their employees who weren't getting any hours.
...
**A5.**
COTTLE CHRISTI L LLC, got the highest loan amount of $280,434. It was approved on February 17th, 2021 and it retained 94 jobs. Customers know it as Kitzie's Cafe and other Kitzie's restaurants. There are 6 locations, two closed, one was during the pandemic on 5/19/2020 and was Kitzie's of Inwood..